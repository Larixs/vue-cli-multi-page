...
const trimTag = function (str){
  if ( typeof (str) == 'string' ) {
    return str.replace(/(<([^>]+)>)/ig, ""); //去除html标签
  } else {
...
const analyseStyleRules = function (styleRules, defaultStyleRules = {}, instanceValue, skipFunction = false){
  let result = {}
  if ( typeof styleRules === "string" ) {//styleRules为string
    result[styleRules] = true
  } else if ( Array.isArray(styleRules) ) {//styleRules为array
    styleRules.forEach(rule =>{
      result[rule] = true
    })
  } else if ( lodash.isFunction(styleRules) ) {//styleRules为function
    result = styleRules(instanceValue)
  } else if ( typeof styleRules === "object" ) {//styleRules为object
    Object.keys(styleRules).forEach(key =>{
      ...
      if ( lodash.isFunction(rule) ) {
        ...
        result[key] = ['', undefined, null].includes(rule) ? defaultStyleRules[key] : rule
      }
    })
  } else {
